...
class Trainer(object):
    ...
    def train_n_iters(self, data, l_data_val):
        ...
        self.model.train()
        # Keep frozen backbone in eval mode even during training
        backbone = getattr(self.raw_model.encoder, 'backbone', None)
    ...
    def evaluate(self, data):
        ...
        count, miss, fp = 0, 0, 0
        vis_hit_count = 0   # frames with predictions
        vis_miss_count = 0  # frames without predictions
            ...
            for bi in range(bs):
                ...
                if n_pred_i == 0:
                    ...
                    if vis_miss_count < 5:
                        ...
                        self._save_eval_vis(vis, data, f"miss{vis_miss_count:02d}")
                        vis_miss_count += 1
                    continue
                ...
                if vis_hit_count < 10:
                    ...
                    self._save_eval_vis(vis, data, f"hit{vis_hit_count:02d}")
                    vis_hit_count += 1
    ...
    def _save_eval_vis(self, vis, data, vis_idx):
        idx_str = vis_idx if isinstance(vis_idx, str) else f"{vis_idx:02d}"
        fn = os.path.join(self.args.visu_dir,
                          f"eval_iter{self.current_iter:07d}_{data.dataset.name}_{idx_str}.jpg")
        Image.fromarray(vis).save(fn)
        if self.writer is not None:
            self.writer.add_image(
                f"{data.dataset.name}-{data.dataset.split}/sample_{idx_str}",
...
def main(args):
    ...
    resume_epoch, resume_iter, resume_best_val = 0, 0, 0.0
    resume_ckpt = None
    ...
    if args.resume is not None and os.path.isfile(args.resume):
        ...
        resume_iter = resume_ckpt.get('iter', 0)
        resume_best_val = resume_ckpt.get('best_val', 0.0)
        if is_main_process():
            print(f"Resumed epoch={resume_epoch}, iter={resume_iter}, best_val={resume_best_val:.4f}")
            print(f"{log}")
    elif args.pretrained is not None and os.path.isfile(args.pretrained):
        if is_main_process():
    ...
    if args.backbone.startswith('dinov3_') or 'dinov3' in args.backbone:
        for param in raw_model.encoder.backbone.parameters():
            param.requires_grad = False
        raw_model.encoder.backbone.eval()
        if is_main_process():
            frozen = sum(p.numel() for p in raw_model.encoder.backbone.parameters())
    ...
    l_val_data.append(DataLoader(
        EndoVisDataset(
            root_dir=args.endovis17_dir, split='valid',
    ...
    l_val_data.append(DataLoader(
        EndoVisDataset(
            root_dir=args.endovis18_dir, split='valid',
            training=False, img_size=args.img_size,
            subsample=args.val_subsample,
        ),
        batch_size=args.val_batch_size, num_workers=args.num_workers,
    ...
    if args.eval_only:
        ...
        ds_needle_puncture = RARPInstanceDataset(
            ...
            min_dice=[args.min_dice_shaft,
                      args.min_dice_wrist,
                      args.min_dice_gripper],
            train_ratio=1.0,#args.needle_train_ratio,
            subsample=1, #args.train_subsample,
        )
        ds_knotting = RARPInstanceDataset(
            ...
            min_dice=[args.min_dice_shaft,
                      args.min_dice_wrist,
                      args.min_dice_gripper],
            train_ratio=0.9, #args.needle_train_ratio,
            subsample=args.train_subsample,
        )
        ...
        ds_endovis17 = EndoVisDataset(
            root_dir=args.endovis17_dir, split='train',
            training=True, img_size=args.img_size,
            subsample=1,
        )
        ds_endovis18 = EndoVisDataset(
            root_dir=args.endovis18_dir, split='train',
            training=True, img_size=args.img_size,
            subsample=1,
        )
        ...
        if is_main_process():
            ...
            print(f"EndoVis17 train:       {len(ds_endovis17)}")
            print(f"EndoVis18 train:       {len(ds_endovis18)}")

        train_dataset = ConcatDataset([ds_rarp50, ds_needle_puncture, ds_needle_grasping, ds_knotting, ds_suture_pulling, ds_endovis17, ds_endovis18])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, default='instrument_dinov3_debug')
    parser.add_argument('--pretrained', type=str, default=None)
    parser.add_argument('--resume', type=str, default=None,
                        help='path to checkpoint to resume training from (restores epoch/iter/optimizer)')
    ...
    parser.add_argument('--endovis17_dir', type=str, default='/mnt/nas/haofeng/data/Ref-Endovis17')
    parser.add_argument('--endovis18_dir', type=str, default='/mnt/nas/haofeng/data/Ref-Endovis18')
    parser.add_argument('--min_dice_shaft',   type=float, default=0.7)
    parser.add_argument('--min_dice_wrist',   type=float, default=0.5)
    parser.add_argument('--min_dice_gripper', type=float, default=0.5)
    ...
    parser.add_argument('--val_subsample', type=int, default=5)
