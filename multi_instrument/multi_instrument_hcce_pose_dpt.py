...
class MultiInstrumentHCCEPoseDPT(nn.Module):
    """
    New model for front-only HCCE CSE plus instrument articulation/wrist heads.

    Existing CSE/DPT model classes are intentionally not modified.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if backbone.startswith("dinov3_") or "dinov3" in backbone:
            raise ValueError("MultiInstrumentHCCEPoseDPT requires DINOv2 EncoderCSE.")
        ...
        self.hcce_dpt_head = DPTHCCEHead(
            embed_dim=self.encoder.embed_dim,
            feat_dim=hcce_feat_dim,
            out_channels=3 * hcce_bits,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        part_embs = self.part_head(y).reshape(-1, self.num_parts, self.mask_dim)
        part_mask_logits = self._compute_part_masks(part_embs, pixel_feat, counts, values)

        hcce_global_logits = self.hcce_dpt_head(
    ...
    def _compute_part_masks(self, part_embs, pixel_feat, counts, values):
        split_sizes = [int(c.item()) for c in counts]
        embs_split = torch.split(part_embs, split_sizes, dim=0)
        all_masks = []
        for emb, batch_idx in zip(embs_split, values):
            pf = pixel_feat[batch_idx]
            all_masks.append(torch.einsum("npd,dhw->nphw", emb, pf))
        return torch.cat(all_masks, dim=0)
