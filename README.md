...
CUDA_VISIBLE_DEVICES=4,5,6,7 /mnt/iMVR/daiyun/anaconda3/envs/multiipr/bin/torchrun \
  --nproc_per_node=4 \
  train_instrument.py \
  --backbone dinov2_vits14 \
  --img_size 896 \
  ...
  --max_iter 60000 \
  -lr 5e-5 \
  --name instrument_s_896_4gpu

# single GPU
conda run -n multiipr python train_instrument_rarp.py --name rarp_combined

# multi-GPU
torchrun --nproc_per_node=4 train_instrument_rarp.py --name rarp_combined

# eval only on both val sets
python train_instrument_rarp.py --eval_only 1 --pretrained logs/rarp_combined/checkpoints/best.pt
